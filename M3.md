...
| Hydra | <code>hydra</code> | <img src='images/hydra.png' width="40" height="40" /> | Hydra are creatures that only spawn in hard mode. When a hydra is attacked by the character or allies, there is a 50% chance that its health will increase rather than decrease by the attacking damage as two heads have grown back when one is cut off.  |
...
| Entity    | JSON Prefix | Image         | Description |
| --------- | ----------- | --------------| ------------|
| Sun Stone | <code>sun_stone</code> | <img src='images/sun_stone.png' /> | Can be picked up by the player, and can be used to open doors and interchangeably with treasure. The player retains the stone after usage. |
...
| Entity    | Image         | Description       |
| --------- | --------------| ------------------|
| Sceptre   | <img src='images/sceptre.png' /> | Can be crafted with one wood or two arrows, one key/treasure, and one sun stone. A character with a sceptre does not need to bribe mercenaries or assassins to become allies, as they can use the sceptre to control their minds. The effects only last for 10 ticks. |
| Midnight Armour | <img src='images/midnight_armour.png' /> | Can be crafted with an armour and a sun stone if there are no zombies currently in the dungeon. Midnight armour provides extra attack damage as well as protection. |
...
In this extension, instead of specifying an existing dungeon to play, players can choose specify a dungeon to be automatically generated when creating a new game, along with a series of entities that are required to appear in the dungeon.
...
<li>If any of <code>requiredEntities</code> are not valid entities</li>
</ul>
</td>
</tr>
</table>
...
As part of this, you will need to be able to automatically generate dungeons. The dungeon needs to be playable and have all of the given entities and be of the given size.
...
function RandomizedPrims(width, height, start, end):
    ...
    add to options all neighbours of 'start' that are of distance 2 away and are walls
    add to options all neighbours of 'end' that are of distance 2 away and are walls
    ...
    while options is not empty:
        ...
        if neighbours is not empty:
            ...
            maze[ position between next and neighbour ] = empty (i.e. true)
            maze[ neighbour ] = empty (i.e. true)

            add to options all neighbours of 'neighbour' that are of distance 2 away and are walls
```

Or, in a more wordy fashion;

- Given a grid that consists of a 2D array of states (Wall/Empty) initialised to only walls
- Set the start and end positions to empty spaces
- Add to a list of positions to process the neighbours of start/end (that are walls)
...
In addition, there is a new type of door which can be opened by a switch.
