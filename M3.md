...
| Entity    | JSON Prefix | Image         | Description |
| --------- | ----------- | --------------| ------------|
| Sun Stone | <code>sun_stone</code> | <img src='images/sun_stone.png' /> | Can be picked up by the player, and can be used to open doors and interchangeably with treasure. The player retains the stone after usage.  Stone takes priority over treasure but if you wish to change this make sure to clarify that in assumptions. |
...
All entities which interact via switches (switches, doors, bombs, and light bulbs), as well as switches themselves can (but are not required to) be created with an additional field (specified in the dungeon JSON) called `"logic"`, which is one of:
